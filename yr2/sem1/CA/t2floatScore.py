...
def checkRec(tDict,rec,avr):
    tally=0
    tally=recScore(tDict,rec)
    
    if rec[14] == "<=50K":
        return (tally <= avr)
    else:
        return not (tally <= avr)
...
downDict={"Age":0,"EdNum":0,"CapG":0,"CapL":0,"Hours":0}
upDict={"Age":0,"EdNum":0,"CapG":0,"CapL":0,"Hours":0}

totalRecs=tallyRecs(f)
print(totalRecs)
accur,over,under,tUp,tDown,tAvr,i=0,0,0,0,0,0,0
...
for row in f:    
    ...
    if part[14] == "<=50K":
        tDown+=recScore(clasDict,part)
    else:
        tUp+=recScore(clasDict,part)

tDown/=under
tUp/=over
tAvr=((int)(((tUp+tDown)/2)*1000)/1000)

print("\navrUp: ",tUp)
print("avrDown: ",tDown)
print("average score: ",tAvr)

i=0
for row in f:    #use last 25% of file
    i+=1 
    if len(row) <= 1: continue
    part=row.strip("\n").split(", ")
    
    accur+=checkRec(clasDict,part,tAvr)

accur/=i

print("\naccuracy: ",round(accur*100),"%")


print("\nGuessed Correctly:",(int)(accur*i),"of",(i))
print("Records trained:",i)
print("Records tested:",totalRecs-i)
